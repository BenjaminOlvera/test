...
import src.globals
from src.swapper import process_video, process_img, process_faces, process_frames
from src.utils import is_img, detect_fps, set_fps, create_video, add_audio, extract_frames, rreplace
from src.analyser import get_face_single
import src.ui as ui
...
if 'all_faces' in args:
    src.globals.all_faces = True

if args.cpu_cores:
    src.globals.cpu_cores = int(args.cpu_cores)

# cpu thread fix for mac
if sys.platform == 'darwin':
    src.globals.cpu_cores = 1

if args.gpu_threads:
    src.globals.gpu_threads = int(args.gpu_threads)

# gpu thread fix for amd
if args.gpu_vendor == 'amd':
    src.globals.gpu_threads = 1

if args.gpu_vendor:
    src.globals.gpu_vendor = args.gpu_vendor
else:
    src.globals.providers = ['CPUExecutionProvider']
...
def pre_check():
    ...
    if src.globals.gpu_vendor == 'apple':
        if 'CoreMLExecutionProvider' not in src.globals.providers:
            quit("You are using --gpu=apple flag but CoreML isn't available or properly installed on your system.")
    if src.globals.gpu_vendor == 'amd':
        if 'ROCMExecutionProvider' not in src.globals.providers:
            quit("You are using --gpu=amd flag but ROCM isn't available or properly installed on your system.")
    if src.globals.gpu_vendor == 'nvidia':
...
def process_video_multi_cores(source_img, frame_paths):
    n = len(frame_paths) // src.globals.cpu_cores
    if n > 2:
        processes = []
...
def start(preview_callback = None):
    ...
    if is_img(target_path):
        if predict_image(target_path) > 0.85:
            print(predict_image(target_path))
        #    quit()
    ...
    if src.globals.gpu_vendor is None and src.globals.cpu_cores > 1:
        global POOL
        POOL = mp.Pool(src.globals.cpu_cores)
...
def toggle_all_faces_handler(value: int):
    src.globals.all_faces = True if value == 1 else False
...
def run():
    ...
    window = ui.init(
        {
            'all_faces': src.globals.all_faces,
